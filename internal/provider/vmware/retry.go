...
// backoffDelay computes the delay for attempt n using full-jitter exponential
// backoff: delay = min(maxDelay, baseDelay * 2^(n-1)).
...
// isRetryableOperationError returns true for transient vSphere operation errors
// that are safe to retry (e.g. concurrent task conflicts, resource busy).
...
// with the standard operation retry policy (3 attempts, 1s base delay).
